...
def endline_sideline(full_surf = True, rotate = False, rotation_dir = 'ccw'):
    ...
    end line and sideline as specified in the field diagram of the NFL rule
    book (Rule 1, Section 1, Article 2)
...
def goal_line(full_surf = True, rotate = False, rotation_dir = 'ccw'):
    """
    Generate the dataframe for the points that comprise the bounding box of the
    goal line as specified in the field diagram of the NFL rule book (Rule 1,
    Section 2, Article 3)
...
def yard_markings(full_surf = True, rotate = False, rotation_dir = 'ccw'):
    ...
    NFL rule book (Rule 1, Section 2, Articles 1 and 2)
    ...
    # 5-yard intervals, a 2' tall by 4" wide marker should be placed 8" from
    # the interior of the sideline as well as 70'9" from the interior of the
    ...
    for yardage in yardages:
        if yardage == 0:
            ...
            # field, with the inbound line marker 70'9" from the interior of
...
def try_line(full_surf = True, rotate = False, rotation_dir = 'ccw'):
    """
    Generate the dataframe for the points that comprise the bounding box of the
    try line as specified in the field diagram of the NFL rule book (Rule 1,
    Section 2, Article 2)
...
def directional_arrows(full_surf = True, rotate = False, rotation_dir = 'ccw'):
    ...
    that are located at every 10-yard interval as described on Page 5 of the
    NFL rule book
...
def field_numbers(ax, full_surf = True, rotate = False):
    ...
    """
    number_font_fpath = os.path.join('data', 'fonts', 'nfl', 'clarenbd.ttf')
